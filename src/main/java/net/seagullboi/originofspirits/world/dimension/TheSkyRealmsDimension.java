...
package net.seagullboi.originofspirits.world.dimension;
...
public class TheSkyRealmsDimension extends OriginofspiritsModElements.ModElement {
	@ObjectHolder("originofspirits:the_sky_realms_portal")
	public static final CustomPortalBlock portal = null;

	public TheSkyRealmsDimension(OriginofspiritsModElements instance) {
		super(instance, 288);
	...
	public void init(FMLCommonSetupEvent event) {
		...
		replaceableBlocks.add(TOOSBlocks.BLESSED_STONE.get());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("originofspirits:glass_dessert")).getGenerationSettings()
		...
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("originofspirits:deceptive_islands")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getUnder().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("originofspirits:sacred_forest")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getTop().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("originofspirits:sacred_forest")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getUnder().getBlock());
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("originofspirits:the_sky_realms"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
...
*/
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("the_sky_realms_portal", Vector3i::compareTo, 300);
	...
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("the_sky_realms_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())),
					0, 1).setRegistryName("the_sky_realms_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new TheSkyRealmsItem().setRegistryName("cursed_steel_and_tempest"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("the_sky_realms_portal");
		}
		...
		public void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote
					&& TheSkyRealmsCanTravelThroughPortalProcedure.executeProcedure(Stream.of(new AbstractMap.SimpleEntry<>("entity", entity))
				...
				if (entity.func_242280_ah()) {
					...
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("originofspirits:the_sky_realms")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == TOOSBlocks.DECEPTONE.get();
		};
		...
		public static PortalInfo func_242963_a(ServerWorld world, TeleportationRepositioner.Result result, Direction.Axis axis, Vector3d offsetVector,
			...
			double d0 = result.width;
			double d1 = result.height;
				...
				for (int l1 = -1; l1 < 2; ++l1) {
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0 ? TOOSBlocks.DECEPTONE.get().getDefaultState() : Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, TOOSBlocks.DECEPTONE.get().getDefaultState(), 3);
					}
				}
			}
