...
public class ClamStructureStructure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
			feature = new Feature<NoFeatureConfig>(NoFeatureConfig.field_236558_a_) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if (!dimensionCriteria)
						return false;
					if ((random.nextInt(1000000) + 1) <= 99000) {
						...
						for (int a = 0; a < count; a++) {
							...
							if (blockAt.getBlock() == TOOSBlocks.WHITE_SAND.get())
								blockCriteria = true;
							if (blockAt.getBlock() == net.seagullboi.originofspirits.registry.TOOSBlocks.ABYSSAL_STONE.get())
								blockCriteria = true;
							if (blockAt.getBlock() == net.seagullboi.originofspirits.registry.TOOSBlocks.PINK_SAND.get())
								blockCriteria = true;
							if (!blockCriteria)
								continue;
							...
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("originofspirits", "clam_structure_enitites"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("clam_structure"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("originofspirits:clam_structure"), configuredFeature);
		}
	}
