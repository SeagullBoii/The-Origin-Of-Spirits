...
public class BigAlcyoneumStructure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
					...
					if ((random.nextInt(1000000) + 1) <= 600000) {
						...
						for (int a = 0; a < count; a++) {
							...
							if (blockAt.getBlock() == net.seagullboi.originofspirits.registry.TOOSBlocks.PINK_SAND.get())
								blockCriteria = true;
							if (blockAt.getBlock() == TOOSBlocks.ABYSSAL_STONE.get())
								blockCriteria = true;
							if (!blockCriteria)
								continue;
							...
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("originofspirits", "big_alcyoneum"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("big_alcyoneum"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("originofspirits:big_alcyoneum"), configuredFeature);
		}
	}
