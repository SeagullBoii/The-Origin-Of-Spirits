...
import net.seagullboi.originofspirits.entity.renderer.ClamEmptyRenderer;
...
public class ClamEmptyEntity extends OriginofspiritsModElements.ModElement {
	public static EntityType entity = (EntityType.Builder.<CustomEntity>create(CustomEntity::new, EntityClassification.WATER_CREATURE)
			.setShouldReceiveVelocityUpdates(true).setTrackingRange(64).setUpdateInterval(3).setCustomClientFactory(CustomEntity::new).size(1f, 0.7f))
					.build("clam_empty").setRegistryName("clam_empty");

	public ClamEmptyEntity(OriginofspiritsModElements instance) {
		super(instance, 251);
		FMLJavaModLoadingContext.get().getModEventBus().register(new ClamEmptyRenderer.ModelRegisterHandler());
	...
	public void initElements() {
		...
		elements.items.add(() -> new SpawnEggItem(entity, -4747932, -2177135, new Item.Properties().group(OriginOfSpiritsEntitiesItemGroup.tab))
				.setRegistryName("clam_empty_spawn_egg"));
	}
	...
	public void addFeatureToBiomes(BiomeLoadingEvent event) {
		...
		if (!biomeCriteria)
			return;
		event.getSpawns().getSpawner(EntityClassification.WATER_CREATURE).add(new MobSpawnInfo.Spawners(entity, 50, 4, 5));
	}
	@Override
