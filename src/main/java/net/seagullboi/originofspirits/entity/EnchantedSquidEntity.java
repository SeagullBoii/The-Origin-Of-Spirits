...
public class EnchantedSquidEntity extends OriginofspiritsModElements.ModElement {
	public static EntityType entity = (EntityType.Builder.<CustomEntity>create(CustomEntity::new, EntityClassification.WATER_CREATURE)
			.setShouldReceiveVelocityUpdates(true).setTrackingRange(64).setUpdateInterval(3).setCustomClientFactory(CustomEntity::new).size(0.6f, 3f))
					.build("enchanted_squid").setRegistryName("enchanted_squid");

	public EnchantedSquidEntity(OriginofspiritsModElements instance) {
		super(instance, 246);
		FMLJavaModLoadingContext.get().getModEventBus().register(new EnchantedSquidRenderer.ModelRegisterHandler());
	...
	public void initElements() {
		...
		elements.items.add(() -> new SpawnEggItem(entity, -9981998, -9781567, new Item.Properties().group(OriginOfSpiritsEntitiesItemGroup.tab))
				.setRegistryName("enchanted_squid_spawn_egg"));
	}
	...
	public static class CustomEntity extends SquidEntity {
		...
		public boolean attackEntityFrom(DamageSource source, float amount) {
			...
			EnchantedSquidEntityIsHurtProcedure.executeProcedure(Stream
		...
		public void onDeath(DamageSource source) {
			...
			EnchantedSquidEntityDiesProcedure.executeProcedure(Stream
					.of(new AbstractMap.SimpleEntry<>("world", world), new AbstractMap.SimpleEntry<>("x", x), new AbstractMap.SimpleEntry<>("y", y),
							new AbstractMap.SimpleEntry<>("z", z))
					.collect(HashMap::new, (_m, _e) -> _m.put(_e.getKey(), _e.getValue()), Map::putAll));
		}
