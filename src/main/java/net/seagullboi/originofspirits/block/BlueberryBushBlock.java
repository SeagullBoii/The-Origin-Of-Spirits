...
public class BlueberryBushBlock extends OriginofspiritsModElements.ModElement {
	@ObjectHolder("originofspirits:blueberry_bush")
	public static final Block block = null;

	public BlueberryBushBlock(OriginofspiritsModElements instance) {
		super(instance, 349);
	}
	...
	public static class CustomBlock extends BushBlock implements IGrowable {
		...
		public void onEntityCollision(BlockState state, World worldIn, BlockPos pos, Entity entityIn) {
			if (entityIn instanceof LivingEntity && entityIn.getType() != EntityType.FOX && entityIn.getType() != EntityType.BEE) {
				...
				if (!worldIn.isRemote && state.get(AGE) > 0
		...
		public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn,
			...
			if (!flag && player.getHeldItem(handIn).getItem() == Items.BONE_MEAL) {
				...
				spawnAsEntity(worldIn, pos, new ItemStack(BlueBerriesItem.block, j + (flag ? 1 : 0)));
